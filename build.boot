...
(deftask encode-version
  ...
  [v version-ns VER sym "Namespace with 'version value."]
...
(deftask build
  "Build jar file suitable for clojars.
  This jar contains only uncompiled files (*.clj) and might be treated as a source one."
  []
  (comp
   (pom)
   ...
   (with-pre-wrap fs
     (core/commit! (let [{:keys [version-sem project]} (meta fs)
                         file-name (format "%s-%s.jar" project version-sem)
                         new-fs (core/mv fs "project.jar" file-name)]
                     (util/info "Write final jar file: %s\n" file-name)
                     new-fs)))
   (target)
   (install)))
